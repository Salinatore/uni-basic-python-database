...
def build_user_operations(gui):
    return [
        Operation("Visualizzare tutte le stanze contenute in un immobile", False, ["codice immobile"],
                  threaded_handler(gui, get_rooms_from_building_code)),
        Operation("Visualizzazione dei materiali utilizzati per un determinato abito", False, ["codice abito"],
                  threaded_handler(gui, get_materials_from_dress_code)),
        Operation("Creazione di un gruppo di lavoro", True, ["data inizio lavoro", "descrizione", "tipo gruppo"],
                  threaded_handler(gui, insert_work_group)),
        Operation("Visualizzazione di tutti gli abiti indossati da una modella", False, ["codice modella"],
                  threaded_handler(gui, get_dresses_from_model_code)),
        Operation("Aggiunta di un partecipante a un evento", False, ["codice evento", "CF persona", "costo"],
                  threaded_handler(gui, insert_participation)),
        Operation("Visualizzazione dei gruppi di lavoro che hanno occupato un ufficio", True, ["numero stanza"],
        ...
        Operation("Visualizzazione di tutti gli abiti indossati in un evento", False, ["codice evento"],
        ...
        Operation("Prenotazione turni di lavoro in una stanza per un determinato gruppo", False, [
            "data inizio turno", "data fine turno", "codice immobile", "codice piano",
            "codice stanza", "codice lavoro", "descrizione"
        ], threaded_handler(gui, insert_work_shift)),
        Operation("Aggiunta di un nuovo acquisto", False, [
            "id spesa", "data e ora", "costo", "CF (opzionale)",
            "codice contrattuale (opzionale)", "codice materiale (opzionale)", "altro campo (opzionale)"
        ], threaded_handler(gui, insert_new_expense)),
        Operation("Visualizzazione delle ore fatte da un dipendente nel suo attuale gruppo di lavoro",
                  True, ["CF dipendente"],
                  threaded_handler(gui, get_total_hours_worked_by_employee)),
        Operation("Individuazione del gruppo di lavoro con il personale più retribuito (attuale e passato)",
                  False, [],
                  threaded_handler(gui, get_highest_paid_work_group)),
        Operation("Cambio gruppo per un dipendente specializzato", True, ["CF dipendente", "nuovo codice lavoro"],
